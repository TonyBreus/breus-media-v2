...
export const MultiContactWidgetV15 = () => {
    ...
    // Скрываем виджет на первом экране (показываем только после scrollY > 400px)
    useEffect(() => {
        const handleScroll = () => {
        ...
        <AnimatePresence>
            {showWidget && (
                <motion.div
                    ...
                    className="fixed bottom-6 right-4 z-[99] flex items-center"
                >
                    <AnimatePresence>
                        {isBannerVisible && (
                            <motion.div
                                ...
                                onClick={handleClick}
                                className="absolute right-13 sm:right-15 top-1/2 -translate-y-1/2 bg-[#111]/95 backdrop-blur-md border border-white/15 text-white px-4 py-2 rounded-full shadow-2xl flex items-center gap-2.5 whitespace-nowrap cursor-pointer hover:border-emerald-500/40 transition-colors"
                            >
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.94 }}
                        className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-emerald-500 hover:bg-emerald-400 text-white shadow-[0_4px_16px_rgba(16,185,129,0.35)] flex items-center justify-center cursor-pointer z-10 p-2 sm:p-2.5"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            fill="currentColor"
