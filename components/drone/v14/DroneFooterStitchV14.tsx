...
type DroneFooterStitchV14Props = {
    missionText?: string;
    menuTitle?: string;
    contactTitle?: string;
    lang?: 'ru' | 'en';
...
export const DroneFooterStitchV14 = ({
    missionText,
    menuTitle,
    contactTitle,
    lang,
}: DroneFooterStitchV14Props) => {
    ...
    return (
        <DebugWrapper id={11000} label="Drone Footer Section V14">
            <footer className="w-full border-t border-white/10 bg-[#050505] py-4 md:py-5">
                <div className="mx-auto w-full max-w-7xl px-4 md:px-6">
                    ...
                    <div className="flex flex-col items-center justify-between gap-3 pb-3 md:flex-row md:gap-4 border-b border-white/10">
                        {/* Menu & 4 Clickable Links wrapped in semantic <nav> */}
                        <nav aria-label={isEn ? 'Service navigation' : 'Навигация по услугам'} className="flex flex-wrap items-center justify-center gap-x-2.5 gap-y-1.5 md:justify-start">
                            <span className="text-[#FFD23F] font-bold text-xs uppercase tracking-[0.2em]">
                                {resolvedMenuTitle}
                            </span>
                            <span className="text-[#FFD23F] text-xs font-bold select-none" aria-hidden="true">•</span>
                    ...
                    <div className="flex flex-col items-center gap-3 pt-3 md:flex-row md:justify-between md:gap-4">
                        <div className="flex items-center text-left text-[18px] font-black uppercase tracking-tighter text-white md:w-[25%]">
                            <Link href={isEn ? "/gazeta/en" : "/gazeta"} className="hover:opacity-85 transition-opacity">
                                BREUS MEDIA <span className="text-[#FFD23F] font-extrabold" aria-hidden="true">•</span>
                            </Link>
                        </div>
