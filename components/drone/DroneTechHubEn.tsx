...
import { techDataEn } from './droneTechData';

type Tab = 'recent' | 'gear' | 'compare';

export const DroneTechHubEn = () => {
        ...
        switch (activeTab) {
                        ...
                        <table className="w-full min-w-[600px] border-collapse text-left text-sm md:text-base">
                            <thead>
                                <tr className="border-b border-[#2a2a2a] text-[#8a8a8a]">
                                    <th className="pb-3 pl-4 pr-4 font-medium">Date</th>
                                    <th className="pb-3 px-4 font-medium">Project / Client</th>
                                    <th className="pb-3 px-4 font-medium">Location</th>
                                    <th className="pb-3 px-4 font-medium">Format</th>
                                </tr>
                            </thead>
                            <tbody className="text-white/90">
                                {techDataEn.recentProjects.map((project, idx) => (
                                    <tr
                                        key={idx}
            ...
            case 'gear':
                return (
                    ...
                    >
                        {techDataEn.gearList.map((item, idx) => (
                            <div
                                key={idx}
                            ...
                            >
                                ...
                                <p className="text-[14px] leading-relaxed text-white/60">
                                    <strong className="text-white/80">Purpose:</strong> {item.purpose}
                                </p>
                            </div>
                        ))}
                    </motion.div>
                        ...
                        <table className="w-full min-w-[600px] border-collapse text-left text-sm md:text-base">
                            <thead>
                                <tr className="border-b border-[#2a2a2a] text-white">
                                    <th className="pb-4 pl-4 pr-4 font-medium w-1/3">Feature</th>
                                    <th className="pb-4 px-4 font-medium w-1/3">Standard Drone</th>
                                    <th className="pb-4 px-4 font-medium w-1/3 text-[#D4A017]">FPV Drone</th>
                                </tr>
                            </thead>
                            <tbody className="text-white/80">
                                {techDataEn.comparison.map((row, idx) => (
            ...
            <div className="container mx-auto px-6">
                <div className="mb-10 max-w-2xl">
                    <p className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-[#D4A017]">
                        <Zap className="h-3.5 w-3.5" /> Database
                    </p>
                    <h2 className="text-3xl font-bold tracking-tight text-white md:text-4xl">
                        Technical Hub
                    </h2>
                    <p className="mt-4 text-[15px] leading-relaxed text-white/60">
                        Facts that matter for B2B clients and productions. We work transparently: showing recent cases, revealing our gear list, and helping you choose the right format.
                    </p>
                </div>
