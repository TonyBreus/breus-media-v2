...
export default function HeaderV15() {
    ...
    return (
        ...
        >
            <div className={`container mx-auto px-6 flex justify-between items-center transition-all duration-300 ${isScrolled ? 'h-[80px]' : 'h-[100px]'} relative z-[60]`}>
                            ...
                            <AnimatePresence>
                                {hoveredMenu === "industries" && (
                                    <motion.div
                                        ...
                                        className="absolute top-[60px] -left-10 w-[600px] bg-black/95 backdrop-blur-xl border border-white/20 p-8 shadow-2xl z-[70] grid grid-cols-2 gap-x-8 gap-y-3 rounded-2xl"
                                    >
                                        {industries.map(item => (
                            ...
                            <AnimatePresence>
                                {hoveredMenu === "services" && (
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, y: 0, x: 0 }}
                                        exit={{ opacity: 0, y: 10, x: 0 }}
                                        className="absolute top-[60px] -left-10 w-[800px] bg-black/95 backdrop-blur-xl border border-white/20 p-8 shadow-2xl z-[70] grid grid-cols-3 gap-x-8 gap-y-3 rounded-2xl"
                                    >
                                        {services.map(item => (
                                            <Link key={item} href="#" className="text-gray-400 hover:text-white hover:translate-x-2 transition-all text-sm uppercase font-semibold block">
                                                {item}
                ...
                {/* RIGHT GROUP: Actions (CTA, Contact, Lang) */}
