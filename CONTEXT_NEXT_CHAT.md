...
- Deployed isolated route `app/drone-service-v13/page.tsx` inheriting full production-ready funnel. Excluded `/drone-service-v13` from global WhatsApp button. Validated clean build across 129 pages.
...
- Re-architected Variant 2 into a seamless monolithic glass capsule: eliminated the divider seam between ticker and header by applying unified `bg-black/25 backdrop-blur-xl border-b border-white/[0.08]` to the outer container, removing heavy drop shadow and decreasing darkness from 65% to 25% for maximum lightness. Preserved fixed layout geometry (`h-6 md:h-7` pre-reserved, opacity transition) completely eliminating header jump (Layout Shift = 0) while keeping ticker above header. Enhanced ticker gold brightness (`#F59E0B`).
- Calibrated `DroneHeroV11.tsx` vertical metrics (`pt-20 md:pt-28`, optical lift `-translate-y-6 sm:-translate-y-10 md:-translate-y-12`) for balanced golden ratio placement.
- Excluded `/drone-service-v11` in `FloatingWhatsAppButton.tsx`. Verified 0 errors across 127 pages.
✅ Hero Lift (10-15%) & B2C Integration in `/drone-service-v10` (2026-09-13): Created isolated `DroneHeroV10.tsx` with optical vertical container lift (`-translate-y-8 sm:-translate-y-12`, top padding calibrated to `pt-16 md:pt-24`) preventing visual sagging towards the bottom screen edge on mobile (390px). Updated sublead copy with approved B2C & private projects integration: «Съёмка на дроны DJI снаружи и манёвренные FPV внутри помещений в Тбилиси и регионах Грузии. 4K-видео и фото для недвижимости, ресторанов, отелей, брендов, а также частных объектов и личных проектов. Готовые материалы от 24 часов.» Synchronized page metadata description and validated clean Next.js production build.
✅ Drone Service Evolution V2–V10 & Full Production Synchronization (2026-09-12 / 2026-09-13): (1) V2: L3-архитектура «Айсберг» для /drone-services/drone-real-estate-v2 (30 факторов в компактный блок с переключением Accordion/Swipe-таблица, сохранение SEO через CSS-скрытие); (2) V3: Изолированный роут /drone-service-v3, новая шапка, Hero без тяжелой боковой карусели, A/B плавающий виджет связи (глобальная кнопка WhatsApp отключена на этих роутах); (3) V4: Прозрачная шапка при скролле, возврат переключателя языка [RU] в мобильной шапке, тест 3 вариантов виджетов (выбран Вариант А: темный компактный с золотым акцентом); (4) V5: Связка Hero ➔ Прогрев: клик по «Смотреть направления» скроллит к #proof-metrics (цифры и доказательства), убран лишний текст перед каталогом; (5) V6: Мобильная эргономика: Hero поднят, кнопка «Тарифы» перенесена под метрики, из мобильной шапки ссылка на тарифы убрана в бургер-меню, виджет связи скрыт на первом экране (появляется при scrollY > 180px); (6) V7: Восстановление привычного первого экрана: возврат кнопки «Тарифы» в Hero под «Смотреть направления», сохранение скрытого виджета и компактной шапки; (7) V8: Горизонтальный свайп-каталог карточек (w-[85vw] snap-center) по образцу v1 с горизонтальной строкой чипов категорий; (8) V9: Статичная сетка категорий 2x2 вместо горизонтального скролла чипов, заголовок «Направления съёмки» (все элементы первого экрана каталога помещаются в 1 экран смартфона без вертикальной прокрутки); добавлен блок «Айсберг» (нативные HTML <details>) с регламентами и базой знаний; (9) V10: Полномасштабная синхронизация и финализация воронки по образцу v1: блок «Дополнительно к любому выезду» (Reels от 150 ₾, 4K от 250 ₾, 360° от 100 ₾, экспресс +50%), чистые 4 шага процесса работы, глубокая фактура «Айсберга» на 5 вкладок (6 бизнес-задач с болями и решениями, регламент GCAA и техника, таблица Air 3S vs Avata 2, 6 стандартов продакшна, полный реестр 18 направлений с перелинковкой на L3), изолированный DronePricingV10 с 5 оригинальными тарифами (200 ₾ – 700 ₾) и смарт-ссылками в WhatsApp с предзаполненным тарифом и ценой (2026-09-13)
✅ Pathname-Based i18n Routing, Header Links Parity, Cross-Page English Retention & Footer Auto-Detection: (1) Fixed logo language reset bug in `SmartHeader.tsx` where clicking top-left logo while on English page routed to `/gazeta` (RU) instead of `/gazeta/en`. Introduced pathname-based language resolution (`const isPathEn = Boolean(pathname && (pathname === "/gazeta/en" || pathname.endsWith("/en") || pathname.includes("/en/"))); const routeLanguage = (normalizedInitialLang === "EN" || isPathEn) ? "EN" : ...`); (2) Fixed all dropdown industry and service navigation links in `headerCopy.EN` so they route to canonical `/en` pages (`/drone-service/en`, `/360-tours-service/en`, `/ai-visualization-service/en`, `/reels-service/en`, etc.), eliminating accidental redirection to Russian pages; (3) Added `localStorage` (`breus_lang`) and `document.cookie` (`NEXT_LOCALE`) persistence to both desktop and mobile language switchers; (4) Added `usePathname()` fallback to `GazetaMinimalFooter.tsx` and `DroneFooterStitch.tsx` so footer logo and menu automatically resolve to English when on an `/en` route; (5) Added `initialLang="en"` to `app/360-tour-hotels/en/page.tsx` (2026-09-11)
✅ Tablet Navigation Breakpoint, Master Footer Unification, iPad Hero Kinetic Fit, Step 04/05 Titles & Cookie Locale Preservation: (1) Fixed tablet navigation disappearance bug (768px-1024px) in `SmartHeader.tsx` by upgrading burger button and drawer overlay to `lg:hidden`; (2) Upgraded `GazetaMinimalFooter.tsx` and unified with `DroneFooterStitch.tsx` / `DroneFooterStitchEn.tsx` under master 2-row standard (top menu row + contact pill with pulsing green indicator `+995 501 103 183`, bottom branding, mission, and social icons with safe right padding `pr-0 sm:pr-16 md:pr-20` preventing floating WhatsApp overlap, purged legal boilerplate); (3) Synchronized steps 04 and 05 across `NichesStack.tsx` (RU & EN) to `04 REELS & SHORTS` and `05 AI-ВИЗУАЛИЗАЦИЯ` (RU) / `05 AI VISUALIZATION` (EN) including all services directory links; (4) Calibrated iPad Pro 11 / tablet kinetic watermark in `HeroSection.tsx` with responsive `yStart` (64vh) and `md:tracking-[0.10em]` to prevent bottom clipping in mobile Safari & Chrome; (5) Smoothed Step 14 `FormCard` scroll behavior with `overscroll-y-contain`, purged jumpy `snap-start`, added bottom safe padding (`pb-16 md:pb-24`), and passed `lang={lang}` to `GazetaMinimalFooter`; (6) Implemented cross-session cookie locale preservation in `app/page.tsx` (`NEXT_LOCALE=en` redirects to `/gazeta/en`) and wrapped service card links in `enRoute()` (2026-09-11)
✅ Fixed language persistence bug across navigation & re-architected Russian Hero H1: (1) SmartHeader logo link now dynamically detects active language and routes to `/gazeta/en` when on English, eliminating unwanted reset to Russian `/gazeta`; (2) In `LangSetter.tsx` enabled persistence to `localStorage` (`breus_lang`) and `document.cookie` (`NEXT_LOCALE`); (3) Re-architected Russian H1 in `HeroSection.tsx` with balanced lines (`["АГЕНТСТВО ВИЗУАЛЬНОГО", "ПРОДАКШЕНА", "И DIGITAL-РЕШЕНИЙ"]`), extended container width to `max-w-[28ch] md:max-w-[34ch]`, removed negative vertical offset (`-top-2.5`), and set fluid typography `text-[clamp(1.3rem,4.5vw,3.3rem)] leading-[1.06]` — completely eliminating text clipping, strange word breaks, and collision with the lower description card during window resize; (4) Documented all changes in `CODEX.md` and `CONTEXT_NEXT_CHAT.md` (2026-09-11)
✅ Standardized 17 Drone Service cards, fixed intermediate tablet layout collapse, and enriched GEO/AEO entities: (1) In `DroneServicesStitch.tsx` and `DroneServicesStitchEn.tsx`, moved titles into the image overlay with bottom gradient across all 17 cards, eliminating blank header space; (2) Removed `hidden md:block lg:hidden` intermediate tablet collapse in Drone Hero, making 2-column layout smoothly scale down to 768px (`md`); (3) Added full list of Tbilisi districts (Vake, Saburtalo, Didi Dighomi, Old Tbilisi, Ortachala, etc.) and Georgia regions in Drone FAQ, flight notes, and Schema.org `areaServed`; (4) Synchronized marquee ticker across RU/EN to `AI-Визуализация` / `AI Visualization` and `Reels & Shorts` (2026-09-11)
...
## UPDATE — 2026-09-19 (drone-service-v14 Mobile UX & Friction Fixes)
- Контекст: Доработка мобильной эргономики L2 страницы аэросъёмки (V14) под Thumb Flow (iPhone 375–393px).
- Внедрённые правки:
  1. **Hero**: добавлен `scroll-mt-24 md:scroll-mt-28` для правильного якорного скролла под плавающую шапку.
  2. **Каталог (тексты)**: H2 оптимизирован под 1 строку (`text-[22px] sm:text-2xl`), подзаголовок изменён на конверсионный («18 профильных решений...»).
  3. **Каталог (табы)**: на мобильном внедрена двухколоночная сетка (`grid-cols-2`), длинный таб занял всю ширину.
  4. **Удаление пустот**: паддинги между каталогом и тарифами сокращены (`pb-24` -> `pb-6`, `py-24` -> `py-10`).
  5. **Тарифы (горизонтальный свайп)**: на мобильных экранах карточки тарифов переведены в горизонтальную карусель с `snap-x` (ширина `82vw`), сэкономив 1200px вертикального пространства.
  6. **Аддоны (компактность)**: карточки переведены в строчный формат (`flex-col sm:flex-row`), добавлено усечение текста (`line-clamp-1`), кнопка переименована в «Заказать» с прямым линком на WhatsApp.
  7. **Процесс (SSR Аккордеон)**: 4 шага на мобильном свернуты в нативный HTML-аккордеон `<details>`, сохранив тексты для краулеров.
  8. **FAQ (Плавность)**: убран рывок высоты `<details>`, внедрен state-based аккордеон с CSS-гридом (`grid-rows-[0fr]/[1fr]`), сохранив SSR-рендеринг текстов внутри.
- Итог: Сборка `131/131` (без ошибок). Правки отправлены в Vercel Preview ветку `main`.

## UPDATE — 2026-09-19 (drone-service-v14 Refactoring: Catalog, Addons, Iceberg)
- **Каталог (табы)**: Удален таб "Все", счетчики позиций. Дефолтным выбран 'real-estate-land'. Внедрена ровная сетка 2x3 на мобильных экранах.
- **Аддоны**: Карточки переделаны в сверхкомпактные строчные компоненты с ценником вместо иконки и описанием через line-clamp-1. Удалены кнопки. Вмещаются в 1 экран.
- **Айсберг (GCAA)**: Удален бейдж, очищен заголовок, удален подзаголовок (строгий B2B стиль).
- Сборка прошла без ошибок, изменения в Vercel Preview.

## UPDATE — 2026-09-20 (drone-service-v15)
- Создана изолированная версия страницы `app/drone-service-v15/page.tsx` (на базе V14) для А/В-теста.
- Заменен виджет. Написан с нуля `MultiContactWidgetV15.tsx`:
  * Справа внизу круглая кнопка WhatsApp (sticky).
  * Динамически выезжающая плашка («Lower Third») с текстом "Остались вопросы? Отвечаем за 5 минут".
  * Появляется через 10 секунд на 4.5 секунды.
  * Цикл показа повторяется раз в 40 секунд (лимит — 3 раза за сессию на странице).
  * Framer motion анимации (opacity/scale/x).

## UPDATE — 2026-09-24 (drone-service-v15 WhatsApp Widget & Catalog Compaction)
- `MultiContactWidgetV15.tsx`:
  * Скрыт на первом экране (`window.scrollY <= 400px`), появляется плавно через `AnimatePresence`.
  * Размер уменьшен до `w-11 h-11 sm:w-12 sm:h-12` (44px / 48px).
  * Реализовано органическое «дыхание» прозрачности (100% -> 50% -> 100%) за 3.5 секунды через `framer-motion`.
- `DroneServicesCatalogV15.tsx`:
  * Кнопки категорий стали низкопрофильными (`py-1.5`, `min-h-[34px]`).
  * Убраны `border-b` и `pb-4` под сеткой табов на мобильных экранах.
...
- **Hero-секция (`DroneHeroV16.tsx`)**:
  * Устранен конфликт двух тяжелых кнопок. Вторая кнопка («Тарифы») стала контурной (`border-white/20`).
  * Подзаголовок сокращен с 3 до 1 конверсионного предложения.
- **Каталог (`DroneServicesCatalogV16.tsx`)**:
  * Убрана пустота в карточках (удален `min-h-[48px]`).
  * На карточках "soon" убрана серая кнопка-заглушка "В разработке", заменена на одну общую кнопку WhatsApp.
  * Бейдж "Готово к просмотру" заменен на "• Страница услуги".
  * Тексты AEO-капсул категорий сокращены до 2 строк, слово "визуал" заменено на "материалы для публикации".
- **Навигация**: Подтверждено наличие `#pricing` («Тарифы») в мобильном дровере `DroneHeaderV16.tsx`. Глобальный виджет WA скрыт на новом роуте.
