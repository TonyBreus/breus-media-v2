...
- Домен: breus.media
- Последний кодовый коммит (локально): 976ee51
...
- В этом репозитории `git remote` пока не настроен, поэтому live может не содержать последние локальные фиксы.
...
2. Подключить рабочий `git remote` (или deploy hook/token), чтобы включить стабильный pipeline публикации.
