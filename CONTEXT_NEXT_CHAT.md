...
- Re-architected Variant 2 into a seamless monolithic glass capsule: eliminated the divider seam between ticker and header by applying unified `bg-black/65 backdrop-blur-md border-b border-white/10` to the outer container. Preserved fixed layout geometry (`h-6 md:h-7` pre-reserved, opacity transition) completely eliminating header jump (Layout Shift = 0) while keeping ticker above header. Enhanced ticker gold brightness (`#F59E0B`).
