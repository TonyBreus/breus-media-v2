...
- Lifted running ticker to the very top layer as an always-visible Announcement Bar (`bg-black/90 backdrop-blur-md border-b border-white/5` with `#D4A017` gold typography), removing scroll threshold dependency (`scrollY > 120`) so dynamic motion starts instantly on load.
- Re-architected lower header tier with responsive `backdrop-blur-md` on scroll, preserving desktop navigation, language switch, call CTA, and mobile drawer.
- Created `DroneHeroV11.tsx` with compensated top padding (`pt-28 md:pt-36`) and optical center lift (`-translate-y-2 sm:-translate-y-4 md:-translate-y-6`), preventing any overlap of H1/sublead/CTAs with the taller header.
- Provisioned isolated route `app/drone-service-v11/page.tsx` inheriting full V10 funnel (addons, 4-step process, 5-tab Iceberg, and WhatsApp-deep-linked pricing). Excluded `/drone-service-v11` from global WhatsApp button. Validated clean build across 127 pages.
