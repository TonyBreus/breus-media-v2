...
- feat(drone-v11-glass): полностью ликвидирован шов между строкой и шапкой; устранена тяжелая чернота — затемнение снижено с 65% до 25% (`bg-black/25`), блюр усилен до ультраматового `backdrop-blur-xl`, убрана тяжелая тень. Шапка стала максимально воздушной и легкой, не съедая пространство первого экрана. Текст строки усилен ярким золотом `#F59E0B`.
...
- meta(drone-v10): обновлен description в `app/drone-service-v10/page.tsx` в соответствии с новым позиционированием.
...
### Session Summary
- Выполнена дополнительная полировка `/drone-hotels-tourism`: добавлены блоки для GEO-цитирования, региональная секция и предвыбор услуги в форме.
- Тексты страницы очищены от упоминаний Батуми в контентных блоках.

### Изменения
- `app/drone-hotels-tourism/page.tsx`
  - `problemCards`:
    - добавлена 6-я карточка:
      - `«Думаю об этом уже полгода, но всё никак не соберусь»`
  - Добавлен тип `StatCard` и массив `statsCards` (4 карточки со stat/label/description/source/sourceUrl).
  - Добавлена секция `Что говорят цифры`:
    - `DebugWrapper id={385115}`.
    - Позиция: сразу после `How It Looks Section` и до `Problems Section`.
  - Обновлены `nicheCards`:
    - `Курортный отель (Батуми / Чёрное море)` → `Винный отель в Кахетии` (+ новые `pain/solution`).
    - `Горный / эко-отель (Казбеги, Местиа, Кахетия)` → `Горный отель (Казбеги, Гудаури, Бакуриани)`.
  - Секция `Почему Грузия — особый визуальный продукт`:
    - абзац про Батуми заменён на:
      - `Гудаури и Бакуриани — горнолыжные склоны зимой, альпийские луга летом...`
  - `faqItems`:
    - ответ `Работаете ли вы за пределами Тбилиси?` обновлён:
      - `Кахетия, Казбеги, Гудаури и Бакуриани...`
  - `whyUsCards`:
    - в карточке `Вид из окна — главный кадр` убрано упоминание Батуми.
  - Добавлена секция `Где мы снимаем в Грузии`:
    - `DebugWrapper id={385365}`.
    - Позиция: после `Related Services Section` и до `PRE-CONTACT CTA`.
    - 6 карточек регионов: Тбилиси, Кахетия, Казбеги/Степанцминда, Гудаури, Бакуриани, апарт-отели по Грузии.
  - Контактная секция:
    - `<DroneContactStitch preselectedServices={['drone']} />` для предвыбора аэросъёмки.

- `components/drone/DroneContactStitch.tsx`
  - Добавлен опциональный проп:
    - `preselectedServices?: string[]`.
  - Введена модель услуг с `id` и `label`:
    - например `id: 'drone'`, `label: 'Аэросъёмка'`.
  - Инициализация `services` теперь учитывает `preselectedServices` (матч по `id` или `label`).
  - Логика рендера чипов переведена на `serviceOptions` с `id`.
  - Для страниц без пропа поведение формы остаётся прежним.

### Verification
- `npm run build` — ✅ успешно, без ошибок.
- `problemCards` содержит 6 элементов.
- Проверка по `rg`:
  - контентные упоминания Батуми удалены;
  - единственное вхождение `Батуми` осталось в `serviceSchema.areaServed`, т.к. schema-объекты по требованиям не менялись.

## 2026-04-17 (polish: drone-hotels-tourism final cleanup, FAQ dedupe, mid-CTA)

### Session Summary
- Выполнена финальная полировка страницы `/drone-hotels-tourism` как эталона для L3-дрон-страниц.
- Удалены дубль-блоки FAQ, вычищены стоп-слова в Hero, добавлен mid-CTA после блока ниш.
- Прокинуты `languageLinks` в `SmartHeader` и `heroId` в вызов `DroneStickyCta` на уровне страницы.

### Изменения
- `app/drone-hotels-tourism/page.tsx`
  - Hero:
    - переписаны 2 финальных абзаца;
    - в Hero заменено упоминание `Google Business Profile` на `карточку в Google Maps`.
  - `problemCards`:
    - удалена карточка `«Ищем оператора с понятной сметой и общением на английском»`;
    - итог: 5 карточек вместо 6.
  - `deliverablesDefault`:
    - удалён пункт `Разрешение на полёт (GCAA) — оформляем сами`;
    - добавлены/обновлены пункты:
  ...
  - `whyUsCards`:
    - заголовок карточки обновлён на `Быстрый контакт, без лишних слоёв` (без `(универсальный)`).
  - `faqItems`:
    - удалён дубль-вопрос `Вы работаете с иностранными отелями и сетями?`;
    - добавлены 3 новых вопроса:
      - `Как дрон помогает выделиться на Booking и похожих площадках?`
      - `Что снимать, если отель ещё строится или открывается?`
      - `Можно ли снять экстерьер зимой и интерьер отдельно летом?`
    - `faqSchema` остался синхронным через `faqItems.map(...)`.
  - Удалены неиспользуемые блоки:
    - `seoAnswers` + секция `Extended FAQ Section` (`DebugWrapper id={385350}`)
    - `shortQA` + секция `Short Answers Section` (`DebugWrapper id={385355}`)
  - Добавлена новая mid-CTA секция сразу после `Niches Section` и до `Georgia & Safety Section`.
  - `SmartHeader`:
    - добавлен `languageLinks`:
      - `RU: /drone-hotels-tourism`
      - `EN: /drone-hotels-tourism/en`
  - `DroneStickyCta`:
    - передан `heroId` из страницы (`drone-hotels-tourism-hero`) без правок компонента.

### Verification
- `npm run build` — ✅ успешно, без ошибок.
- Чек по ключевым требованиям выполнен через собранный output (`.next/server/app/drone-hotels-tourism.html`) и проверку исходника:
  - Hero обновлён, целевые формулировки применены.
  - В `problemCards` осталось 5 карточек.
  - В `deliverablesDefault` отсутствует строка про GCAA.
  - Добавлен mid-CTA после `Niches Section`.
  - Удалены секции `Подробные ответы на частые вопросы` и `Коротко о главном`.
  - В FAQ присутствуют 3 новых вопроса.

## 2026-04-17 (drone-restaurants + drone-hotels-tourism: content sync, FAQ and why-us rewrite)

### Session Summary
- Выполнена контентная унификация страниц `/drone-services/drone-restaurants` и `/drone-hotels-tourism`.
- На ресторанах мобильный вид блока «Сравнение пакетов» приведён к той же модели, что на отелях (единая таблица со скроллом).
- Обновлены блоки «Как выбрать пакет», FAQ и «Как мы подходим к съёмке» в обеих страницах.
- Удалён нерелевантный FAQ-блок про `Film in Georgia rebate` на странице отелей.

### Изменения
- `app/drone-services/drone-restaurants/page.tsx`
  - Удалён отдельный mobile-карточный рендер сравнения пакетов (`md:hidden`), оставлена единая таблица с `overflow-x-auto`.
  - Обновлён блок `Как выбрать пакет` (формулировки 4 сценариев + финальная строка).
  - FAQ:
    - добавлены верхние вопросы:
      - `Что именно вы снимаете для ресторанов?`
      - `Можно ли снять дрон, интерьер и короткие вертикальные ролики в одном проекте?`
      - `Подходят ли материалы для сайта, Instagram, Reels, рекламы и Google Business Profile?`
    - добавлен вопрос `Есть ли договор?` с новым ответом;
    - удалены вопросы:
      - `Где ещё можно использовать то, что мы снимем?`
      - `Что именно даёт дрон-съёмка ресторану?`
    - обновлены ответы по срокам съёмки/выдачи и географии выезда.
  - Полностью обновлён массив `whyUsCards`:
    - новая формулировка заголовков/описаний;
    - добавлена карточка `Снимаем в рабочие часы ресторана`.
- `app/drone-hotels-tourism/page.tsx`
  - В таблице сравнения пакетов обновлены строки:
    - `Фото` (`10+ / — / 10+ / 20+`)
    - `Видео для Google Maps`
    - `Время на объекте` (`~1,5 ч / ~1,5 ч / ~2,5 ч / по задаче`)
  - Обновлён блок `Как выбрать пакет` (включая формулировку про Google Maps и площадки размещения).
  - FAQ:
    - удалён блок `Как получить Film in Georgia rebate?`;
    - `Вы работаете с иностранными отельными сетями?` → `Вы работаете с иностранными отелями и сетями?` + новый ответ;
    - обновлён ответ для `Есть ли договор?`.
  - Обновлён FAQ-ответ про форматы съёмки: убраны брендовые упоминания дронов (`Air 3S`, `Avata 2`).
  - Обновлён ценовой FAQ-абзац на новую версию (цены/4K/аддоны).
  - Полностью обновлён массив `whyUsCards` под новую отельную версию:
    - акцент на бронирование, Booking/Google Maps, вид из окна и долгосрочную актуальность контента.
...
### Изменения
- `app/drone-service/page.tsx`
  - `openGraph.locale`: `ru_RU` → `ru_GE`.
  - Pricing-секция обёрнута в `<div id="pricing">...</div>`, чтобы навигация по `#pricing` попадала в нужный блок.
  - В `ItemList` schema исправлен URL для `Туризм и локации`:
    - было: `https://breus.media/drone-hotels-tourism`
    - стало: `https://breus.media/drone-services/drone-territory`
- `components/gazeta/SmartHeader.tsx`
  - В `InteractiveTicker` вычисляется `copyIndex` для повторных циклов ticker-элементов.
  - Только первый логический цикл оставлен доступным для assistive tech.
  - Все повторные копии помечены как декоративные:
    - `aria-hidden="true"`
    - для duplicate-link копий также добавлен `tabIndex={-1}`
- `components/drone/DroneFAQExpanded.tsx`
  - Обновлён ответ на вопрос `Работаете ли вы за пределами Тбилиси?`
  - В текст добавлены районы Тбилиси: `Ваке, Сабуртало, Мтацминда, Дидубе, Глдани, Исани, Варкетили`.
  - Уточнено, что отдельно обсуждается стоимость выезда за пределы Тбилиси.
- `components/drone/DroneHeroStitch.tsx`
  - Внутрь hero-секции перед фоновым блоком добавлен скрытый семантический `h1`:
    - `Аэросъёмка дроном для бизнеса в Тбилиси — Breus Media`
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущим SEO/accessibility правкам.
...
## 2026-04-12 (docs-sync(cards): локальная картинка фасадов и финальный порядок 18 карточек)
### Session Summary
- В docs зафиксирован финальный pass по карточкам направлений: карточка `Инспекция фасадов` переведена на локальную картинку, а все 18 направлений расставлены в утверждённом порядке.

### Изменения
- `components/drone/droneServicesData.ts`
  - Для `Инспекция фасадов` источник изображения обновлён:
    - было: внешний `Unsplash` URL
    - стало: `/media/drone-service/drone-facade-2.png`
  - Через `order` зафиксирован финальный порядок карточек:
    - `Недвижимость`
    - `Мониторинг стройки`
    - `Отели & Курорты`
    - `Рестораны`
    - `Регулярные аэроотчёты`
    - `Земельные участки`
    - `Реклама и бренд видео`
    - `Автосалоны и шоурумы`
    - `Инспекция фасадов`
    - `Туризм и локации`
    - `Мероприятия`
    - `Инспекция объектов`
    - `FPV Съёмка`
    - `Съёмка интерьеров и складов`
    - `Инспекция солнечных панелей`
    - `Спорт комплексы`
    - `Мониторинг территорий`
    - `Агро и виноделие`
- `public/media/drone-service/drone-facade-2.png`
  - Добавлен локальный файл картинки для карточки `Инспекция фасадов`.
- `components/drone/DroneHeroStitch.tsx`
  - Зафиксировано текущее состояние после отката эксперимента: нижний hero-блок снова остаётся компактным списком на 4 пункта, а не сеткой из 18 направлений.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена новая верхняя запись по финальной синхронизации карточек.
...
## 2026-04-12 (docs-sync(hero+services): незадокументированные правки Hero и карточек направлений)
### Session Summary
- Синхронизированы незадокументированные изменения по `/drone-service`: донастройка hero-типографики/индикаторов и полный pass по карточкам направлений (визуал + copy).

### Изменения
- `components/drone/DroneHeroStitch.tsx`
  - Подключён `Manrope` (`next/font/google`) и применён к desktop-блокам:
    - 4 фразы слева под `АЭРОСЪЁМКА ДЛЯ БИЗНЕСА В ГРУЗИИ`
    - описание активной карточки справа
  - Для этих блоков применены мягкие параметры читаемости (`font-normal`, `16px`, `line-height 1.65`, `text-white/[0.88]`, `antialiased`).
  - Кнопка `Открыть услугу` в правом hero-блоке переведена в outlined-стиль (`border-[#D4A017]`, прозрачный фон, жёлтый текст).
  - Мини-карусель справа обновлена:
    - удалены точки-маркеры
    - активный пункт: жёлтый текст + подложка `bg-[#D4A017]/16`
  - Блок стрелки приведён к текущему виду:
    - `18 направлений` (без `↓` в тексте)
    - `от 250 ₾ · Тбилиси · Батуми · Кутаиси` в жёлтом цвете
    - уменьшен общий размер подписи.
- `components/drone/DroneServicesStitch.tsx`
  - Контейнер секции карточек расширен до `max-w-[1400px]` при сохранении прежних промежутков (`gap-5`).
  - Убран визуальный верхний заголовок секции `Направления съёмки` (карточки начинаются сразу после отступа секции).
  - Типографика всех карточек унифицирована под стиль тестовых первых двух:
    - title: `Manrope 20/700`
    - audience: `Manrope 11/600`, `tracking 0.12em`, `white/45`
    - description: `Manrope 14/400`, `line-height 1.65`, `white/82`, `antialiased`
    - tags: `Manrope 11/700`, `tracking 0.1em`, `#F5C518`
  - Кнопки карточек возвращены к стандартному виду (`text-[10px]`, стандартные заливка/обводка).
- `components/drone/droneServicesData.ts`
  - Обновлены подзаголовки и/или жёлтые теги для карточек:
    - `Недвижимость` (включая обновление description с `94% ... HomeJab`)
    - `FPV Съёмка`
    - `Отели & Курорты`
    - `Рестораны`
    - `Туризм и локации`
    ...
    - `Инспекция солнечных панелей`
    - `Регулярные аэроотчёты`
  - В текущем diff отсутствует карточка `Инспекция крыш` (`id: 16`) — зафиксировано как актуальное состояние массива.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена синхронизирующая верхняя запись по всему блоку незадокументированных правок.
...
### Изменения
- `components/drone/DroneProcessStitch.tsx`
  - Массив `steps` полностью заменён:
    - `01 ЗАДАЧА`
    - `02 ПОДГОТОВКА`
    - `03 СЪЁМКА`
    - `04 ГОТОВО`
  - Обновлены описания для всех 4 шагов согласно новому тексту.
  - В mobile-блоке строка `Как мы работаем` заменена на `От заявки до готовых файлов`.
  - Структура JSX и классы оставлены без изменений.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущему фиксу.
...
## 2026-04-11 (fix(process-mobile): горизонтальный скролл → вертикальный список со стрелками)
### Session Summary
- В `DroneProcessStitch` mobile-представление шагов процесса переделано из горизонтального скролла в вертикальный линейный список со стрелками между шагами.

### Изменения
- `components/drone/DroneProcessStitch.tsx`
  - Полностью заменён `md:hidden` блок.
  - Добавлен заголовок `Как мы работаем`.
  - Каждый шаг теперь рендерится как компактная карточка с номером, заголовком и описанием.
  - Между карточками добавлены вертикальные разделители `↓`.
  - Desktop-ветка (`hidden md:grid`) не изменялась.
  - Отступы секции `py-4 md:py-24` сохранены.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущему фиксу.
...
- DroneFlightConditionsNote: текст заменён на однострочный про GCAA
- DroneContactStitch: на мобиле скрыты поля «Имя», «Услуги», «Ссылка», дисклеймер; форма влазит в один экран iPhone 14 Pro
- npm run build: ✅
- Статус: local ready

---

## 2026-04-11 (drone-service: add ItemList schema with 18 directions)
### Session Summary
- На `/drone-service` добавлен отдельный JSON-LD `ItemList` schema для L2-хаба услуг аэросъёмки.
- Схема вставлена рядом с текущими schema-скриптами в начале `<main>`, без изменений существующих schema-блоков.

### Изменения
- `app/drone-service/page.tsx`
  - После `{jsonLdSchemas.map(...)}` добавлен новый `<script type="application/ld+json">` с:
    - `@type: ItemList`
    - `name: Услуги аэросъёмки дроном в Тбилиси`
    - `numberOfItems: 18`
    - `itemListElement` из 18 `ListItem` (позиции 1–18) с URL на `https://breus.media/...`
  - Блок размещён до `<DronePageScrollProgress />` как запрошено.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущему schema-обновлению.
...
## 2026-04-11 (drone-service: remove duplicate related label + add source to 68% hero line)
### Session Summary
- На `/drone-service` выполнены 2 точечные текстовые правки: убран дублирующий строчный лейбл в related-блоке и добавлен источник к mobile-строке про `68%`.
- Изменения ограничены только `DroneRelatedLinksCompact` и mobile-частью `DroneHeroStitch`.

### Изменения
- `components/drone/DroneRelatedLinksCompact.tsx`
  - Удалён `<p>`-лейбл над `h2`, который дублировал заголовок.
  - `h2` `Другие услуги Breus Media` оставлен без изменений.
- `components/drone/DroneHeroStitch.tsx`
  - В mobile-блоке (`block md:hidden`) строка обновлена:
    - `Объекты с аэровидео продаются на 68% быстрее.` →
    - `Объекты с аэровидео продаются на 68% быстрее (MLS / NAR).`
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущему мини-батчу.
...
## 2026-04-11 (drone-service: remove roof inspection card from services data)
### Session Summary
- По точечному запросу из `droneServicesData` удалена карточка направления `Инспекция крыш`.
- Остальные карточки и компоненты страницы не изменялись; выполнена проверка сборкой.

### Изменения
- `components/drone/droneServicesData.ts`
  - Удалён объект карточки:
    - `slug: inspekciya-krysh`
    - `title: Инспекция крыш`
  - Другие элементы массива `droneServiceItems` оставлены без изменений.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущей правке.
...
## 2026-04-11 (drone-service related links + roof href + real estate metric update)
### Session Summary
- Для `/drone-service` обновлены related-ссылки под актуальные соседние услуги Breus Media и синхронизирован заголовок related-блока.
- В данных карточек исправлены ссылка `Инспекция крыш` и метрика в описании карточки `Недвижимость`.

### Изменения
- `constants/droneDirectionPages.ts`
  - В `droneDirectionPages.droneService.relatedLinks` полностью заменён массив на 4 пункта:
    - `360° виртуальные туры` → `/360-tours-service`
    - `Reels и короткое видео` → `/reels-service`
    - `Промо-видео` → `/promo-video-service`
    - `AI-визуализация` → `/ai-visualization-service`
  - Для чистоты после замены удалены неиспользуемые деструктурированные роуты (`hotels`, `restaurants`, `tourism`).
- `components/drone/DroneRelatedLinksCompact.tsx`
  - Заголовочная пара текстов обновлена:
    - eyebrow: `Другие услуги Breus Media`
    - `h2`: `Другие услуги Breus Media`
  - Визуальная структура компонента не менялась.
- `components/drone/droneServicesData.ts`
  - Карточка `Недвижимость`: в `description` заменено упоминание `68%` на
    `94% больше просмотров получают объявления с аэрофото — по данным HomeJab.`
  - Карточка `Инспекция крыш`: `primaryHref` изменён на `/drone-services/drone-roof-inspection`.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по этому батчу.
...
## 2026-04-11 (drone-service mobile cleanup: services title removal + compact contact form)
### Session Summary
- По мобильному фидбеку убран лишний заголовок перед карточками услуг и очищен нижний hint-блок направлений.
- Контактная форма на `/drone-service` уплотнена под телефонный экран, чтобы ключевые поля и CTA занимали меньше вертикального пространства.

### Изменения
- `components/drone/DroneServicesStitch.tsx`
  - Удалён заголовок `Направления съёмки` и декоративная золотая линия над сеткой карточек `#services`.
- `components/drone/DroneServicesMobileList.tsx`
  - Удалён двухстрочный поясняющий текст под grid направлений.
  - Сохранён якорный CTA `смотреть карточки` со стрелкой на `#services`.
- `components/drone/DroneContactStitch.tsx`
  - Проведена mobile-компрессия формы:
    - уменьшены паддинги секции/карточки
    - уплотнены интервалы формы и размеры label/input/button/chips
    - уменьшена высота textarea на mobile
  - Desktop-поведение сохранено через `md:`-правила.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по текущему батчу.
...
## 2026-04-10 (drone-service content batch: stats + faq + mobile hint + trust removal)
### Session Summary
- Для `/drone-service` обновлён контентный блок после services: удалён trust-lite, добавлен новый stats strip с 4 метриками и обновлён FAQ до 8 вопросов.
- В mobile-блок направлений возвращён нижний hint с якорем на карточки услуг, чтобы пользователю было проще перейти к детальному описанию.

### Изменения
- `components/drone/DroneServicesMobileList.tsx`
  - Добавлен нижний hint-блок под grid направлений:
    - текст: `Каждое направление — отдельная страница...`
    - CTA-якорь: `href="#services"` с `ChevronDown` и подписью `смотреть карточки`
  - Импорт иконок обновлён: `ChevronRight` + `ChevronDown`.
- `components/drone/DroneStatsStrip.tsx` (new)
  - Добавлен новый блок `Аэросъёмка в цифрах` с 4 карточками:
    - `68%` — `MLS / NAR`
    - `94%` — `HomeJab, 2025`
    - `403%` — `Inman / MLS`
    - `73%` — `NAR`
  - Стили: тёмный фон, gold-акценты, grid `2 -> 4` колонки.
- `app/drone-service/page.tsx`
  - Удалён импорт и рендер `DroneTrustLite`.
  - Добавлен импорт и рендер `DroneStatsStrip`.
  - Новый порядок в зоне services/pricing:
    - `DroneServicesStitch`
    - `DroneStatsStrip`
    - `DronePricingStitch`
- `components/drone/DroneFAQExpanded.tsx`
  - Полностью заменён FAQ-массив на 8 новых вопросов/ответов (стоимость, FPV, география, GCAA, форматы, сроки, ниши, фото-only).
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по этому батчу.
...
## 2026-04-10 (drone-service mobile ux polish: anchors, ctas, messenger icons)
### Session Summary
- По живому UX-фидбеку оптимизирован мобильный флоу `/drone-service`: hero-copy, позиционирование стрелки, компактность блока направлений и поведение якорей.
- Для карточек услуг добавлены две CTA-кнопки, а в mobile bottom bar заменены буквенные заглушки на реальные иконки мессенджеров с фирменными цветами.

### Изменения
- `components/drone/DroneHeroStitch.tsx`
  - Mobile-описание в hero заменено на новый набор из 4 смысловых строк:
    - `С земли не видно главного...`
    - `Объекты с аэровидео продаются на 68% быстрее.`
    - `...FPV-технология.`
    - `Один полёт — контент...`
  - Блок с chevron и подписью переработан:
    - стрелка над текстом
    - подпись: `19 направлений — найдите своё · от 250 ₾`
    - блок поднят выше для предотвращения конфликта с нижней зоной экрана
  - Поведение staged reveal сохранено (fade-in).
- `components/drone/DroneServicesMobileList.tsx`
  - Уплотнён layout блока направлений:
    - уменьшены паддинги/гапы карточек
    - удалён нижний hint-блок
    - заголовок обновлён на `Направления съёмки`
  - Ссылки переключены на внутренние якоря карточек: `#service-[slug]`.
  - Для стабильной посадки по якорю добавлен `scroll-mt-20` на `#directions`.
  - Для нечётного количества элементов (19) последняя карточка растягивается на 2 колонки (убран пустой чёрный блок).
- `components/drone/DroneServicesStitch.tsx`
  - В карточках услуг вместо одной кнопки `Подробнее` добавлены 2 кнопки:
    - `Открыть услугу` (на `detailsHref`)
    - `Обсудить задачу` (на `#contact`)
  - Удалена debug-обёртка заголовка `id={10301}`.
- `components/drone/MobileBottomBar.tsx`
  - Буквы `W` / `T` заменены на SVG-иконки WhatsApp и Telegram.
  - Добавлен фирменный цвет и обводка:
    - WhatsApp: `#25D366`
    - Telegram: `#229ED9`
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись по этому набору правок.
...
- Убраны `Портфолио`, `Оборудование`, `Карта полётов`; оставлен только рабочий пункт `Контакты`.
- Изменение внесено в общий футер-компонент, который подключён на страницах проекта.

### Изменения
- `components/drone/DroneFooterStitch.tsx`
  - В массиве пунктов меню удалены:
    - `Портфолио`
    - `Оборудование`
    - `Карта полётов`
  - `Контакты` (`href="#contact"`) оставлены без изменений.
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена запись о фактическом удалении пунктов меню.
- `CHANGELOG_ARCHIVE.md`
  - Добавлена эта append-only запись.

### Build
- Не запускался: точечная правка одного компонента футера
...
- Цель: единая упаковка пакетов и одинаковая логика presentation на уровне дрон-направлений.

### Изменения
- `components/drone/DronePricingStitch.tsx`
  - Перенесена структура карточек под стиль restaurants:
    - `title`, `subtitle`, `price`, `items`, `note`, CTA `Обсудить этот пакет →`
  - Синхронизированы 4 пакета:
    - `Полёт снаружи` — `250 ₾`
    - `Пролёт внутри (FPV)` — `350 ₾`
    - `Полная съёмка` — `500 ₾` (бейдж `Выгодно`)
    - `Съёмка с готовым результатом` — `от 900 ₾`
  - Блок add-ons синхронизирован с restaurants:
    - `Монтаж Reels до 30 сек (музыка + субтитры): +150 ₾`
    - `Монтаж ролика 30–60 сек для сайта: +150 ₾`
    - `360° панорама с воздуха: +80 ₾`
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись о синхронизации pricing.
- `CHANGELOG_ARCHIVE.md`
  - Добавлена эта append-only запись.
...
## 2026-04-10 (drone-service contact form pilot updates)
### Session Summary
- На `/drone-service` обновлена форма обратной связи как пилот перед масштабированием на другие страницы.
- Цель: снизить трение при отправке и быстрее получать контекст по объекту.
...
- `components/drone/DroneContactStitch.tsx`
  ...
  - Поле `Ваше имя` сделано необязательным:
    - убран маркер `*`
    - убран `required`
  - В чипах услуг заменено:
    - `Аэросъемка` → `Инспекция`
  - Порядок `Интересующие услуги` приведён к согласованному:
    - `Недвижимость`, `Отели`, `Рестораны`, `Туризм`, `Инспекция`, `Мониторинг`
- `CONTEXT_NEXT_CHAT.md`
  - Добавлена верхняя запись о form pilot update.
- `CHANGELOG_ARCHIVE.md`
  - Добавлена эта append-only запись.
...
## 2026-09-24: drone-service-v15 Clean Card 2026 & WA Duplicate Fix
- **Clean Card 2026 Standard**: В `DroneServicesCatalogV15` убраны `priceAnchor` и нижние теги из карточек. Внедрена строгая матрица LSI-сущностей (`lsiSubtitle`). Кнопки прижаты сразу под описание.
- **WhatsApp Duplicate Fix**: Отключен глобальный виджет `FloatingWhatsAppButton` на роутах `/drone-service-v14` и `/drone-service-v15` для предотвращения визуального конфликта.

## 2026-09-25: drone-service-v16 Release (Mobile UX & AEO Refactoring)
- **Изоляция**: Создана изолированная страница `/drone-service-v16` с компонентами Hero, Catalog и Header версии V16. Глобальный виджет WA скрыт для V16.
- **Hero**: Оптимизирована иерархия кнопок (главная `bg-[#D4A017]`, вторичная контурная `border-white/20`). Подзаголовок сокращен до 1 предложения.
- **Catalog**: Удален `min-h-[48px]` в описаниях. Скрыта серая кнопка «В разработке», заменена на полноразмерную кнопку WA. Бейдж «Готово к просмотру» изменен на «• Страница услуги».
- **AEO**: Тексты категорий сжаты до 2 предложений, слово «визуал» заменено на «материалы для публикации».
- **Header**: Добавлена/подтверждена ссылка «Тарифы» в мобильное меню.
