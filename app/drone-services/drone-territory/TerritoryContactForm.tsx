...
export function TerritoryContactForm() {
    ...
    return (
        <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
                <label className="mb-2 block text-[11px] font-bold uppercase tracking-[0.2em] text-white/50">Объект и задача</label>
                <textarea
                    rows={4}
                    placeholder="Локация, площадь, желаемая частота вылетов, формат отчёта..."
                    className="w-full rounded-[14px] border border-white/12 bg-white/[0.03] px-4 py-3 text-white outline-none transition-colors placeholder:text-white/28 focus:border-[#FFD23F]/55"
                />
            </div>
